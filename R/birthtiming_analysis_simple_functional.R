...
run_naive_sem <- function(){
  ...
  diff_model_naive_out <- lavaan::sem(model=diff_model_naive,
                                      data=gen_birthtiming_data(min_children=2, max_children=10) %>%
...
#   '
#
#   diff_model_full_out <- lavaan::sem(model=diff_model_full,
#                                      data=gen_birthtiming_data(min_children=2, max_children=10, standardize=TRUE) %>%
#                                        filter(!is.na(difficulty_1) & !is.na(difficulty_2)), missing = "ML")
