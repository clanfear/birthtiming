...
NLSY_var_index <- read.csv("./inst/data/raw_data/NLSY_var_index.csv", stringsAsFactors = F)
...
child_difficulty <- reshape(data=data_subset,
                     varying=list(Difficulty  = str_subset(names(data_subset), "Difficulty"),
                                     ...
                                     direction="long") %>% arrange(mother_id, birth_year, birth_month) %>%
  mutate(Cries=case_when(
    !is.na(Cries_0_11_1986) ~ Cries_0_11_1986,
    !is.na(Cries_12_23_1986) ~ Cries_12_23_1986,
    TRUE ~ as.integer(NA)
  )) %>% select(-race, -m_prebirth_weight, -m_prepreg_weight, -weight_loss_gain, -Cries_0_11_1986, -Cries_12_23_1986, starts_with("prenatal")) %>%
...
names(child_difficulty) <- ifelse(str_sub(names(child_difficulty),1,1) %in% c(1,2,3),
                                  paste0(str_sub(names(child_difficulty),3,-1),"_", str_sub(names(child_difficulty),1,1) ), names(child_difficulty))

child_difficulty_std <- child_difficulty %>%
  mutate_at(vars(matches("(Difficulty|m_weight_change|c_birth_weight_oz)")), funs(standardize))
