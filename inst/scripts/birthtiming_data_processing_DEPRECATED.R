...
nlsy_data_raw       <- nlsy_mother$new_data
nlsy_categories_raw <- nlsy_mother$categories
...
#---------------------------------------------------------------------------------
nlsy_data <- nlsy_data_raw %>% select(one_of(var_index$varcode))
names(nlsy_data) <- var_index$varname[match(names(nlsy_data), var_index$varcode)]
# Add child health data
load("./inst/data/raw_data/bt_child_health.RData")
data_subset <- nlsy_data %>%
